...
import shutil
import time
# from mindspore._akg.akg.tvm.contrib import cudnn
from util import *
...
class Engine(object):
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        input_var = ms.Tensor(self.state['input'])
        target_var = ms.Tensor(self.state['target'])
    ...
    def init_learning(self, model, criterion):
        ...
        if self._state('train_transform') is None:
            normalize = vision.Normalize(mean=model.image_normalization_mean.asnumpy().tolist(),
                                         std=model.image_normalization_std.asnumpy().tolist())
            self.state['train_transform'] = transforms.Compose([
                vision.Crop((0, 0),size=(224,224)),
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None):
        ...
        # train_dataset = train_dataset.batch(self.state['batch_size'])
        # val_dataset = val_dataset.batch(self.state['batch_size'])

        # data loading code
        train_loader = ds.GeneratorDataset(source=train_dataset, column_names=["feature", "label"])

        val_loader = ds.GeneratorDataset(source=val_dataset, column_names=["feature","label"])

        # optionally resume from a checkpoint
        if self._state('resume') is not None:
            if os.path.isfile(self.state['resume']):
                print("=> loading checkpoint '{}'".format(self.state['resume']))
                checkpoint = ms.load(self.state['resume'])
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        feature_var = ms.Tensor(self.state['feature']).float()
        target_var = ms.Tensor(self.state['target']).float()
        inp_var = ms.Tensor(self.state['input']).float()  # one hot
        if not training:
        ...
        self.state['loss'] = ms.tensor(10)

        if training:
            pass
            # optimizer.zero_grad()
            # self.state['loss'].backward()
            # ops.clip_by_norm(model.parameters(), max_norm=10.0)
            # optimizer.step()

    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):

        self.state['target_gt'] = copy.deepcopy(self.state['target'])
        self.state['target'][self.state['target'] == 0] = 1
        self.state['target'][self.state['target'] == -1] = 0
