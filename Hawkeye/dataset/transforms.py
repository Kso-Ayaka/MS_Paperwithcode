...
from msadapter.torchvision.transforms import functional as F
from msadapter.torchvision.transforms import autoaugment, transforms
    ...
    def __init__(
        ...
        trans.extend(
            [
                ...
                transforms.ConvertImageDtype(ms.float),
...
class RandomMixup(ms.nn.Module):
    ...
    def forward(self, data) -> Tuple[ms.Tensor, ms.Tensor]:
        """
        Args:
        ...
        if target.dtype != ms.int64:
            raise TypeError(f"Target dtype should be ms.int64. Got {target.dtype}")
        ...
        if target.ndim == 1:
            target = ms.nn.functional.one_hot(target, num_classes=self.num_classes).to(dtype=batch.dtype)

        if ms.rand(1).item() >= self.p:
        ...
        lambda_param = float(ms._sample_dirichlet(ms.tensor([self.alpha, self.alpha]))[0])
...
class RandomCutmix(ms.nn.Module):
    ...
    def forward(self, data) -> Tuple[ms.Tensor, ms.Tensor]:
        ...
        x1 = int(ms.clamp(r_x - r_w_half, min=0))
        y1 = int(ms.clamp(r_y - r_h_half, min=0))
        x2 = int(ms.clamp(r_x + r_w_half, max=W))
        y2 = int(ms.clamp(r_y + r_h_half, max=H))
