...
class APINet(nn.Module):
    ...
    def forward(self, images, targets=None, flag='train'):
        ...
        if flag == 'train':
            ...
            intra_pairs, inter_pairs = intra_pairs.to(ms.int32), inter_pairs.to(ms.int32)

            features1 = ms.cat([pool_out[intra_pairs[:, 0]], pool_out[inter_pairs[:, 0]]], dim=0)
            features2 = ms.cat([pool_out[intra_pairs[:, 1]], pool_out[inter_pairs[:, 1]]], dim=0)
            labels1 = ms.cat([intra_labels[:, 0], inter_labels[:, 0]], dim=0)
            labels2 = ms.cat([intra_labels[:, 1], inter_labels[:, 1]], dim=0)

            mutual_features = ms.cat([features1, features2], dim=1)
            map1_out = self.map1(mutual_features)
            map2_out = self.drop(map1_out)
            map2_out = self.map2(map2_out)

            gate1 = ms.mul(map2_out, features1)
            gate1 = self.sigmoid(gate1)

            gate2 = ms.mul(map2_out, features2)
            gate2 = self.sigmoid(gate2)

            features1_self = ms.mul(gate1, features1) + features1
            features1_other = ms.mul(gate2, features1) + features1

            features2_self = ms.mul(gate2, features2) + features2
            features2_other = ms.mul(gate1, features2) + features2
            ...
            self_logits = ms.zeros(2 * batch_size, 200)
            other_logits = ms.zeros(2 * batch_size, 200)
    ...
    def get_pairs(self, embeddings, labels):
        ...
        intra_labels = ms.from_numpy(intra_labels)
        intra_pairs = ms.from_numpy(intra_pairs)
        inter_labels = ms.from_numpy(inter_labels)
        inter_pairs = ms.from_numpy(inter_pairs)

        return intra_pairs, inter_pairs, intra_labels, inter_labels


def pdist(vectors):
    distance_matrix = -2 * vectors.mm(ms.t(vectors)) + vectors.pow(2).sum(dim=1).view(1, -1) \
