...
class ChannelInteractionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        # SCI Module
        bilinear_matrix = ms.bmm(x, ms.transpose(x, 1, 2)) / (W * H)  # B C C
        W_SCI = F.softmax(-bilinear_matrix, dim=2)  # B C C

        Y = ms.bmm(W_SCI, x)  # Y -> B C WH
        ...
        # CCI Module
        y = Y.view(B, -1)
        y_a = ms.cat((y[:B // 2], y[B // 2:]), dim=1)
        y_b = ms.cat((y[B // 2:], y[:B // 2]), dim=1)
        eta = self.fc(y_a)
        gamma = self.fc(y_b)

        weight = ms.cat((eta, gamma), dim=0)
        W_SCI_BA = ms.cat((W_SCI[B // 2:], W_SCI[:B // 2]), dim=0)
        W_CCI = ms.abs(W_SCI - weight.view(-1, 1, 1) * W_SCI_BA)

        Y_CCI = ms.bmm(W_CCI, x)  # Y -> B C WH
...
class CINClassifier(nn.Module):
    ...
    def forward(self, x):
        if isinstance(x, tuple):
            Z, Z_CCI = x
            Z = ms.squeeze(self.pool(Z))
            Z = self.classifier(Z)
            return Z, Z_CCI
        else:
            x = ms.squeeze(self.pool(x))
            x = self.classifier(x)
            return x
